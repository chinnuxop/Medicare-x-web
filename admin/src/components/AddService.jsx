...
import { User, AlertTriangle, Clock, XCircle, Plus, Trash2, CheckCircle, Image,Calendar } from "lucide-react";
...
const AddService = ({ serviceId }) => {
    ...
    const [removeImage, setRemoveImage] = useState(false);//exiting ones
    ...
    //user can't select previous year/month/date.
    useEffect(() => {
    ...
    function handleImageChange(e) {
        ...
        //remove the existing image if user choose anew file.
        setRemoveImage(false);
        setHasExistingImage(false);
    }
    ...
    //reset the form back to ionitial state
    function resetForm() {
        if (imagePreview && imagePreview.startsWith("blob:")) {
            try {
                URL.revokeObjectURL(imagePreview);
    ...
    //function selected 12hrs components to a date object
    ...
    //submit function for creating or update
                    ...
                    <div
                      ...
                      ${toast.type === "error"
                        ...
                        <button onClick={() => setToast(null)}
                            ...
                            <XCircle className="w-4 h-4 text-grey-400 hover:text-gray-600" />
                        </button>
                    </div>
                )}
            </div>
