...
export const getAppointmentsByPatient = async (req, res) => {
    try {
        ...
        console.log('resolvedCreatedBy (query or req.quth.userId):', resolvedCreatedBy);
    ...
    try {
        ...
        if (existingBooking) {
            return res.status(409).json({
                success: false,
                message: "You already have an appoinement with this doctor at the selected slot."
            })
        }
        ...
        try {
            ...
            console.log("Doctor lookup failed", e?.message || e)
        }
...
export const getStats = async (req, res) => {
    try {
        ...
        return res.json({
            success: true,
            stats: { total, revenue, recentLast70Days: recent }
        });
    }
    catch (err) {
        console.error("getStatsAppointments Error:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
...
return res.json({success: true,totalUsers});
        ...
        console.error("etRegisteredUserCount Error:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
