...
//So this function takes date time slot string and group them
...
//Safely convert price input to a number, stripping out any non-numeric characters except dot and minus sign
...
export async function getAllServices(req, res) {
    try {
        ...
        const list = await Service.find().sort({
            createdAt: -1
        }).lean();
...
export async function updateService(req, res) {
    ...
    try {
        ...
        //to update each feild if already present then update them
        ...
        console.error("UpdatedService Error:", err);
        return res.status(500).json({
            success: false,
            message: "Server Error"
        });
